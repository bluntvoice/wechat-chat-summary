...
def render_html_report(document: dict[str, Any] | None = None, **legacy_kwargs: Any) -> str:
    ...
    def render_quote(item: Any, resolve_value=resolved) -> str:
        ...
        meta_parts = []
        if speaker:
            meta_parts.append(f'<span class="quote-speaker">{speaker}</span>')
        if sent_at:
            meta_parts.append(f'<span class="quote-time">{sent_at}</span>')
        meta = '<span class="quote-separator"> · </span>'.join(meta_parts)
        return (
...
<style>
:root{{--ink:#23452d;--muted:#6b7f71;--green:#7bc96f;--green-dark:#4d9f5f;--paper:#f8fff4;--yellow:#fff7cf;--pink:#fff0f1;--blue:#edf7fb;--line:rgba(35,69,45,.10);--shadow:0 10px 28px rgba(92,135,83,.10);--topic-body-size:14px;}}
...
.quote-item{{padding:8px 0}} blockquote{{margin:5px 0 0;padding:12px;border-radius:8px;background:linear-gradient(180deg,#fffdf5,#fdf2f8);border:1px solid rgba(225,106,151,.15);font-size:13px;color:#526159}} .quote-item p{{font-size:11px;margin:6px 9px 0}} .quote-meta{{font-size:var(--topic-body-size);line-height:1.6}} .quote-speaker{{color:#3478bd;font-weight:800}} .quote-time,.quote-separator{{color:var(--muted);font-weight:400}}

...
统一固化为稳定成员占位符并解析为蓝色加粗的完整群昵称；唯一昵称匹配可以忽略空白差异，但最终显示始终恢复原始完整昵称，存在跨成员歧义时不猜测。新报告不提取讨论落点和行动事项，Schema 2.2 的 `outcome` 固定为 `null`、`action_items` 固定为空数组；旧讨论落点保留并继续展示，旧行动事项保留但不展示或索引。链接卡片、普通

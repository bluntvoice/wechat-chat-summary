...
同一语义话题可合并多个不连续时间区间，问题、风险、引用和资源嵌入对应话题；必要进展直接写入讨论脉络，新报告不再另设讨论落点。“相关原话”的署名和日期时间与话题正文共用基础字号，署名保持蓝色粗体、时间保持次级颜色；Playwright PNG 直接复用同一 HTML 样式。
...
统一固化为稳定成员占位符并解析为蓝色加粗的完整群昵称；每个主要话题还会按 `evidence_ids` 反查并保留 1–2 位代表成员，避免整段只剩“群友、有人、成员、多人”等泛称；唯一昵称匹配可以忽略空白差异，但最终显示始终恢复原始完整昵称，存在跨成员歧义时不猜测。新报告不提取讨论落点和行动事项，Schema 2.2 的 `outcome` 固定为 `null`、`action_items` 固定为空数组；旧讨论落点保留并继续展示，旧行动事项保留但不展示或索引。链接卡片、普通
...
手动生成、软件内定时生成和历史重新生成均经桌面桥接进入同一 `_generate` / CLI 主流水线，并携带
`manual`、`scheduled` 或 `regenerate` 来源。桌面应用级任务上下文在任务开始时固定群聊、日期和来源，
后端进度只更新同一 `task_id`，不得由当前表单选择覆盖。
...
桌面端的“定时总结管理”使用软件内置定时器，可按稳定 `chat_id` 为多个群聊分别保存一项当日或昨日任务，
支持独立增删改和启停；旧单群配置会幂等迁移。多个到期任务按 `chat_id + report_date` 去重并串行执行，
单项失败不阻断后续任务。软件关闭时不执行，并与本文件后文供 CLI/RPA 场景使用的 Windows 任务计划
注册模块相互独立。

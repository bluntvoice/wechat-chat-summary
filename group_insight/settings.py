...
这里统一管理运行时默认值、环境变量读取和正则模式，避免上层流程直接依赖
零散的全局状态。
"""
...
DEFAULT_ANALYZE_END = ""
# 设为 True 时，脚本生成 PNG 后会自动尝试发送。公开默认值保持关闭，
# 避免首次运行时误发到微信会话。
DEFAULT_SEND_AFTER_RUN = False
# 默认发送目标会话列表；建议通过 --send-target 显式指定。

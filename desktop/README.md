...
- 打开图片、报告数据目录和群聊报告目录；
- 在独立历史中心按群聊、日期、模块和关键词查询本地报告，并在详情预览中直接选择条目生成屏蔽版；
- 在独立热力图页查看消息数、参与人数或有效消息数，并从报告日期跳转历史中心；
- 首次使用显示可关闭的七步快速指南，并可从统一图标导航随时重开；
...
只负责请求身份、交互和日历绘制。生成百分比只读取 Python 阶段事件，耗时由 React 本地时钟持续刷新，
不会伪造阶段进度。历史详情和屏蔽列表使用报告内 `member_aliases` 解析成员昵称，隐藏内部 ID、Schema
和原始时间对象字段。历史屏蔽模式把可见模块映射到后端提供的目标 ID，整项点击为主，折叠列表用于
批量和查漏；提交后仍由 Python 对当前 JSON 重新校验。MCP 设置会显示状态、transport、endpoint、端口和复制配置，

...
- 同一语义话题跨不连续时段时合并并保留多个时间区间；时间相邻不得单独作为话题合并依据，
  ...
  `outcome`、问题、风险、引用和资源为可选嵌套细节；没有可靠内容时不得生成占位文本。
  不得重新引入“关键观点”“讨论转折”或独立的“详细讨论脉络”等重复模块，也不得对外展示“轻松插曲”。
- “今日主要话题”每项必须先显示标题，再以普通正文样式显示时间范围，不使用胶囊外框；今日速览和话题内容中的
  可安全唯一匹配群成员昵称使用蓝色加粗；较长讨论脉络按讨论顺序、成员观点或子问题显示为 2–5 个短段。
- 新报告不生成或展示行动事项，Schema 2.2 的 `action_items` 仅为兼容保留并固定为空数组；旧报告数据不删除，
  但报告渲染、历史模块、筛选、搜索和人工屏蔽入口均不展示行动事项。AI 今日观察不生成“讨论弱点 / 不足 / 短板”。
- MCP 新报告必须先严格校验 Report Schema 2.2，再分配版本和写入文件/HistoryStore；只接受
  `topics[*]` 嵌套的结论、问题、风险、引用和资源关联；`action_items` 必须为空数组。非法报告不得污染 SQLite。
...
- 历史中心默认详情必须与报告一级板块一致：今日总览由详情头部承载，其后为今日速览、今日主要话题、
  AI 今日观察、今日活跃情况和报告结尾。话题内结论、问题、风险、原话及资源不得在默认详情重复拆卡；
  今日活跃情况不得重复详情头部已有的统计总数。

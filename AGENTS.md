...
- 匿名版是 normal report 的本地确定性派生版本，不重新调用 AI；编号来自整个报告范围内真实消息的首次发言顺序，不按昵称、活跃度或正文出现顺序编号。
- 匿名导出使用允许字段集合，不得携带真实成员身份映射、账号或反向映射；不按昵称全文替换。无法可靠匿名的署名移除，明确身份残留阻止导出；普通正文其他敏感信息由用户分享前确认。
- 匿名版通过 `source_report_id` 关联具体 normal revision；重建只替换该派生版，删除正常版同时处理关联匿名版。匿名版不参与热力图 / 已总结日期重复统计。
- 历史报告重新生成必须复用主 generation pipeline，始终使用当前软件版本的 Prompt、生成逻辑和当前 AI 配置；仅支持正常单日报告。
- 重新生成创建最高 revision + 1，不覆盖旧报告或旧匿名版；同群同日任务须防并发，完整导出成功后才写历史，失败清理新产物并保留旧版本。
...
- AI Provider 的模型标识必须允许手动填写任意非空值；当前模型可以作为建议项展示，但不得用固定白名单阻止服务商发布的新模型。已知旧别名可继续规范化，未知标识应去除首尾空白后原样保留。

...
- 手动、定时与历史重新生成共用同一生成流程；进度框始终显示真实任务创建时固定的群聊、日期、来源和后端阶段进度，不随表单切换而改变。
...
模型标识、API URL 和 API Key 均可按服务商当前信息手动填写；模型列表在已有输入时仍可展开，连接测试成功的模型会按 Provider 保存在本机供后续直接选择。DeepSeek 的当前模型建议仅用于辅助输入，不会限制后续新模型。
...
主要话题中的“相关原话”署名和日期时间使用与话题正文相同的基础字号；署名保持蓝色加粗，时间使用次级颜色。HTML 与 PNG 使用同一排版结果。
...
桌面端支持为多个群聊分别设置每日生成时间和“生成当日 / 昨日”规则。任务按稳定群聊 ID 绑定，群名变化不会误绑；同一群聊只保留一项任务，可独立添加、编辑、启停和删除。旧版单群配置会自动迁移且不会重复创建。

多个任务同时到期时会进入串行队列：当前任务完成或失败后再执行下一项，按“群聊 ID + 报告日期”去重。只有「群聊拾遗」保持运行时才会触发；软件关闭期间不会在后台执行，重新打开后仍会根据保存状态判断待执行任务。
...
历史中心支持基于已有正常报告生成匿名版，成员按首次发言顺序显示为“群友01、群友02……”，隐藏真实成员身份并移除成员级个性化统计，不重新调用 AI。匿名版单独保存并关联原版本；多日报告统一编号。

匿名处理仅针对成员身份，不会自动删除聊天正文中的其他敏感信息，分享前仍需自行确认。旧报告缺少发言顺序时需要连接 WeChatDataAnalysis；无法安全处理的成员引用会提示先重新生成正常报告或屏蔽对应内容。

### 14. 历史报告重新生成

正常单日报告可以直接在历史预览页面重新生成，无需返回生成页重新选择群聊和日期；重新生成使用当前版本逻辑和当前 AI 配置创建新的历史版本，并保留旧报告，完成后自动打开新版本。此操作会再次调用 AI，可能产生 API 费用。

### 15. 手动检查与安装更新

关于页只在用户点击后检查本项目官方 GitHub Stable Release。更新对话框显示完整更新内容、真实下载百分比和字节数，并在下载后明确执行 SHA-256 完整性校验；下载、校验或安装程序启动失败时可从对应阶段重试，快速重复点击不会并发执行。

正式版、预发布版和测试包使用显式构建通道。远程正式版数值版本更高时可更新；同一数值版本下，测试包或预发布包也可以升级到正式版；不会自动降级或向普通通道推送 Prerelease。
...
软件自身的“生成总结”调用用户配置的 DeepSeek 或 OpenAI Compatible API。模型标识可直接填写任意非空的服务商当前值，已有文字时仍能展开模型列表；API 连接测试会核验实际响应模型，并把测试成功的模型按 Provider 保存在本机供后续选择。DeepSeek V4 Flash / Pro 只是输入建议。DeepSeek 支持其专属的 Thinking、Reasoning Effort 和余额查询；通用 OpenAI Compatible 请求不会自动携带 DeepSeek 专属字段。

...
历史中心使用本机 SQLite 索引，支持按群聊、日期范围和报告模块筛选，并搜索总结、成员、话题、结论、开放问题、风险、引用、资源标题、文件名和 URL。旧报告中的行动事项数据不会删除，但不再展示、筛选或进入搜索结果。中文搜索在全文索引未命中时还会使用本地子串匹配。

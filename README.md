...
AI 总结、统一结构 JSON、完整 HTML 及 300 DPI PNG 完整长图。v0.2.1 已包含 Windows 桌面端闭环、
...
主要话题采用圆形序号讨论脉络，同一语义话题可保留多个不连续时间区间；结论、行动、
问题、风险、引用和资源嵌入对应话题，无法可靠关联的补充信息统一收在 AI 今日观察中。
...
报告中的成员名称优先使用群昵称，其次使用微信网名，不使用本机联系人备注；群关键词会做
规范化去重。资源整理会在提取阶段忽略微信红包消息、疑似红包领取页与红包素材链接。
...
`YYYY-MM-DD_至_YYYY-MM-DD`。报告 JSON 使用向后兼容的 `report schema 2.1`，与 HTML、PNG 和

...
一个话题即使在当天多个不连续时间段被反复讨论，也可以被归纳到同一个主题中。开放问题、风险、引用和资源会尽可能关联到对应话题；必要进展直接写入讨论脉络，新报告不再另设“讨论落点”。较长讨论脉络会按讨论顺序、成员观点或子问题分成 2–5 个短段；今日速览与主要话题中的成员占位符、唯一昵称和已知账号 ID 会统一显示为蓝色加粗昵称，唯一昵称匹配忽略空白差异，存在跨成员歧义时不会猜测。
